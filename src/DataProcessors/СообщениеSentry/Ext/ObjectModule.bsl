...
КонецПроцедуры // ДозаполнитьТелоСообщения()
 ...
 Функция HTTPСоединениеSentry() Экспорт

    HTTPСоединение  = Новый HTTPСоединение("my_sentry_server", , , , , ,Новый ЗащищенноеСоединениеOpenSSL);
	
	Возврат HTTPСоединение;

КонецФункции // HTTPСоединениеSentry()
 
// Отправляет подготовленное сообщение в сервис Sentyr
//
Функция ОтправитьСообщениеВSentry()

    HTTPСоединение  = HTTPСоединениеSentry();
